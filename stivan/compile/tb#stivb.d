...
== TB#STIVB ~L'Equilibrio? Io non penso che esista una via di mezzo, Jaheira. Se il compito di un sovrano è quello di governare, adempirà al suo dovere con il pugno di ferro o nel più generoso dei modi. Anche mia mamma diceva sempre che non ci sono altre alternative.~
...
== BYOSHIM ~Non proprio, mio piccolo amico. Innanzitutto, quella spada a lama curva e taglio singolo che tu chiami katana in realtà porta il nome di uchigatana. Viene brandita unicamente dai samurai o dai membri della classe guerriera, mentre a tutti gli altri è vietato l'utilizzo.~
...
END
== VALYGARJ IF ~InParty("Valygar") !StateCheck("Valygar",CD_STATE_NOTVALID)~ THEN ~Non so se essere lieto di esserci disfatti di un'altro stregone, o adirato per avere un demone nel gruppo.~
...
== YOSHJ IF ~InParty("Yoshimo") !StateCheck("Yoshimo",CD_STATE_NOTVALID)~ THEN ~Stivan, hai ascoltato i miei insegnamenti? Le donne non vanno insultate finchè non ci sei andato a letto.~
